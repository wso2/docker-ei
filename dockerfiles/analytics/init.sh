...
# replace host name entries in event-processor.xml file with the Docker container IP
sed -i "s#<hostName>integrator-with-analytics-ei-analytics</hostName>#<hostName>${docker_container_ip}</hostName>#" ${WSO2_SERVER_HOME}/wso2/${server_profile}/conf/event-processor.xml
